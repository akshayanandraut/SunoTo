...
import { cleanupFailure,percentile,probeOptions,summarizeRealtimeResults } from "../load/staging-realtime.mjs";
...
describe("staging realtime probe",()=>{
  ...
  it("treats any participant cleanup rejection as a probe failure",()=>{assert.equal(cleanupFailure([{status:"fulfilled",value:{}},{status:"rejected",reason:new Error("failed")}]),"cleanup failed for 1 participant");assert.equal(cleanupFailure([{status:"fulfilled",value:{}}]),null);});
