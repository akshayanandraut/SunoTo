...
describe("privacy operations",()=>{
  ...
  it("uses an audited RPC for deletion request transitions",async()=>{let call;const service=new AdminService({SUPABASE_URL:"https://project",SUPABASE_SERVICE_ROLE_KEY:"service"},async(url,options)=>{call={url,body:JSON.parse(options.body)};return Response.json({status:"processing"});});await service.updateDeletion({adminId:"admin",id:"request",status:"processing",note:"Identity verified"});assert.match(call.url,/rpc\/admin_update_deletion_request$/);assert.equal(call.body.decision_note,"Identity verified");});
