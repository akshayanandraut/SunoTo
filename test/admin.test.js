...
describe("restriction enforcement",()=>{
  ...
  it("excludes stale presence from exact admin counts",async()=>{const shard=new PresenceShard(presenceTestState(),{});shard.presence.set("stale",{status:"waiting",anonymous:true,lastSeen:Date.now()-100000});shard.presence.set("live",{status:"chatting",anonymous:false,lastSeen:Date.now()});const stats=await(await shard.fetch(new Request("https://presence/stats"))).json();assert.equal(stats.waiting,0);assert.equal(stats.chatting,1);assert.equal(stats.registeredOnline,1);});
});
