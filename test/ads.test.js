...
describe("three-tier ad policy",()=>{
  it("shows all free placements and interstitials every fifth scan",()=>{assert.deepEqual(adDecision({scanCount:4,config:enabled}).placements,["top","bottom","desktopSide"]);assert.deepEqual(adDecision({scanCount:5,config:enabled}).placements,["top","bottom","desktopSide","interstitial"]);assert.equal(adDecision({registered:true,balance:0,scanCount:5,config:enabled}).tier,"full_page_ads");});
  it("removes interstitials from balances 1 through 1000",()=>{for(const balance of [1,500]){const result=adDecision({registered:true,balance,scanCount:5,config:enabled});assert.equal(result.tier,"full_page_ads");assert.equal(result.placements.includes("interstitial"),true);}const threshold=adDecision({registered:true,balance:1000,scanCount:5,config:enabled});assert.equal(threshold.tier,"side_ads_only");assert.equal(threshold.placements.includes("interstitial"),false);});
  it("is ad-free only above 1000 Credits and respects the kill switch",()=>{assert.deepEqual(adDecision({registered:true,balance:1001,config:enabled}).placements,["top","bottom","desktopSide"]);assert.equal(adDecision({registered:true,balance:1001,config:enabled}).tier,"side_ads_only");assert.equal(adDecision({config:{...enabled,enabled:false}}).tier,"disabled");});
...
describe("replaceable browser ad provider",()=>{
  ...
  it("loads a versioned public snapshot and rejects an unversioned response",async()=>{const snapshot=await loadPublicAdSnapshot(async()=>Response.json({ads:enabled,version:4,flags:{signup_enabled:true}}));assert.deepEqual(snapshot,{config:enabled,version:4,flags:{signup_enabled:true},membership:null,adEarning:null});assert.deepEqual(await loadPublicAdConfig(async()=>Response.json({ads:enabled,version:4})),enabled);await assert.rejects(()=>loadPublicAdSnapshot(async()=>Response.json({ads:enabled})),/invalid_public_config/);});
});
