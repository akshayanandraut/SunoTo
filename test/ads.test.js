...
describe("versioned ad configuration",()=>{
  ...
  it("requires a separately configured verified super-admin",async()=>{const request=new Request("https://api/admin",{headers:{authorization:"Bearer token"}}),env={ADMIN_USER_ID:"admin-id"};assert.equal((await adminUser(request,env,async()=>null)).error.status,401);assert.equal((await adminUser(request,env,async()=>({id:"other",emailVerified:true}))).error.status,403);assert.equal((await adminUser(request,env,async()=>({id:"admin-id",emailVerified:false}))).error.status,403);assert.equal((await adminUser(request,env,async()=>({id:"admin-id",emailVerified:true}))).user.id,"admin-id");});
