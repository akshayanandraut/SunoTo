...
import { fingerprintLaunchReport,launchReportFingerprint,requireReleaseRevision,writeFingerprintedLaunchReport,writeLaunchReport } from "../scripts/launch-report.mjs";
...
describe("immutable launch reports",()=>{
  ...
  it("returns the verified fingerprint when writing immutable evidence",async()=>{const directory=await mkdtemp(join(tmpdir(),"random-chat-fingerprint-")),path=join(directory,"realtime.json"),report={kind:"staging-realtime",revision:"a".repeat(40)},result=await writeFingerprintedLaunchReport(path,report);assert.equal(result.path,path);assert.match(result.fingerprint,/^[0-9a-f]{64}$/);assert.equal(result.fingerprint,launchReportFingerprint(await readFile(path)));});
