...
import { ArenaLobbyShard } from "./durable/ArenaLobbyShard.js";
...
import { hasPaidPreferences,normalizePaidPreferences,requiresExactMatch } from "./policies/preferencePolicy.js";
import { experienceRequiresVideo } from "./policies/experiencePolicy.js";
...
import { LiveWorldService } from "./services/LiveWorldService.js";
...
async function handleRequest(request, env) {
    ...
    if(url.pathname.startsWith("/api/v1/admin/")){
      const authorization=await adminUser(request,env);if(authorization.error)return authorization.error;const user=authorization.user,config=new ConfigService(env,env.FETCHER||fetch),admin=new AdminService(env,env.FETCHER||fetch);try{
        ...
        if(url.pathname==="/api/v1/admin/pricing"&&(request.method==="GET"||request.method==="PUT")){if(request.method==="GET")return Response.json(await config.pricing());const body=await request.json();if(!Number.isSafeInteger(body.expectedVersion))return Response.json({error:"invalid_config_version"},{status:400});try{return Response.json(await config.updatePricing({adminId:user.id,expectedVersion:body.expectedVersion,config:body.config}));}catch(error){return Response.json({error:error.message},{status:error.message==="config_version_conflict"?409:400});}}
        if(url.pathname==="/api/v1/admin/arena"&&(request.method==="GET"||request.method==="PUT")){if(request.method==="GET")return Response.json(await config.arena());const body=await request.json();if(!Number.isSafeInteger(body.expectedVersion))return Response.json({error:"invalid_config_version"},{status:400});try{return Response.json(await config.updateArena({adminId:user.id,expectedVersion:body.expectedVersion,config:body.config}));}catch(error){return Response.json({error:error.message},{status:error.message==="config_version_conflict"?409:400});}}
        ...
        if(request.method==="GET"&&url.pathname==="/api/v1/admin/live-world-placements")return Response.json({placements:await admin.liveWorldPlacements()});
        if(request.method==="POST"&&url.pathname==="/api/v1/admin/live-world-placements/remove"){const body=await request.json();if(!/^[0-9a-f-]{36}$/i.test(body.userId||"")||typeof body.reason!=="string"||body.reason.trim().length<4)return Response.json({error:"invalid_removal_request"},{status:400});await admin.removeLiveWorldPlacement({adminId:user.id,userId:body.userId,reason:body.reason});return Response.json({ok:true});}
        if(request.method==="GET"&&url.pathname==="/api/v1/admin/store-items")return Response.json({items:await admin.storeItems()});
        if(request.method==="POST"&&url.pathname==="/api/v1/admin/store-items"){const body=await request.json();if(typeof body.item!=="object"||!body.item)return Response.json({error:"invalid_store_item"},{status:400});try{return Response.json({item:await admin.upsertStoreItem({adminId:user.id,item:body.item})});}catch(error){return Response.json({error:error.message},{status:400});}}
        if(request.method==="GET"&&url.pathname==="/api/v1/admin/party-rooms")return Response.json({rooms:await admin.partyRooms(Number(url.searchParams.get("limit")||100))});
        if(request.method==="POST"&&url.pathname==="/api/v1/admin/party-rooms/close"){const body=await request.json();if(!/^[0-9a-fA-F-]{36}$/.test(body.roomPublicId||"")||typeof body.reason!=="string"||body.reason.trim().length<4)return Response.json({error:"invalid_close_request"},{status:400});let room;try{room=await admin.closePartyRoom({adminId:user.id,roomPublicId:body.roomPublicId,reason:body.reason});}catch(error){return Response.json({error:error.message},{status:error.message==="room_not_found"?404:400});}if(env.PARTY_ROOM){const id=env.PARTY_ROOM.idFromName(body.roomPublicId);await env.PARTY_ROOM.get(id).fetch("https://party-room.internal/admin/close",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({reason:body.reason})}).catch(()=>{});}return Response.json({room});}
    ...
    if(request.method==="GET"&&url.pathname==="/api/v1/store/catalog"){
      try{const items=await new StoreService({url:env.SUPABASE_URL,serviceKey:env.SUPABASE_SERVICE_ROLE_KEY,fetcher:env.FETCHER||fetch}).catalog();return Response.json({items});}catch(error){return Response.json({error:error.message},{status:502});}
    }
    if(request.method==="GET"&&url.pathname==="/api/v1/store/inventory"){
      const user=await verifySupabaseUser(request,env);if(!user)return Response.json({error:"invalid_account_session"},{status:401});try{const owned=await new StoreService({url:env.SUPABASE_URL,serviceKey:env.SUPABASE_SERVICE_ROLE_KEY,fetcher:env.FETCHER||fetch}).inventory(user.id);return Response.json({owned});}catch(error){return Response.json({error:error.message},{status:502});}
    }
    if(request.method==="POST"&&url.pathname==="/api/v1/store/purchase"){
      const user=await verifySupabaseUser(request,env);if(!user)return Response.json({error:"invalid_account_session"},{status:401});const limited=await enforceRateLimit(request,env,"store_purchase",user.id);if(limited)return limited;const body=await request.json().catch(()=>({}));const itemId=Number(body.itemId);if(!Number.isSafeInteger(itemId)||itemId<=0)return Response.json({error:"invalid_store_item"},{status:400});const idempotencyKey=`store-purchase:${user.id}:${itemId}`;try{const result=await new StoreService({url:env.SUPABASE_URL,serviceKey:env.SUPABASE_SERVICE_ROLE_KEY,fetcher:env.FETCHER||fetch}).purchase(user.id,itemId,idempotencyKey);return Response.json({balance:result.balance,itemId:result.item_id,idempotent:result.idempotent});}catch(error){return Response.json({error:error.message},{status:error.message==="insufficient_credits"?402:error.message==="invalid_store_item"?404:400});}
    }
    if(request.method==="POST"&&url.pathname==="/api/v1/store/equip"){
      const user=await verifySupabaseUser(request,env);if(!user)return Response.json({error:"invalid_account_session"},{status:401});const body=await request.json().catch(()=>({}));const itemId=body.itemId===null?null:Number(body.itemId);if(itemId!==null&&(!Number.isSafeInteger(itemId)||itemId<=0))return Response.json({error:"invalid_store_item"},{status:400});try{const profile=await new StoreService({url:env.SUPABASE_URL,serviceKey:env.SUPABASE_SERVICE_ROLE_KEY,fetcher:env.FETCHER||fetch}).equip(user.id,itemId);return Response.json({equippedBadgeItemId:profile.equipped_badge_item_id});}catch(error){return Response.json({error:error.message},{status:error.message==="item_not_owned"?403:400});}
    }
    if(request.method==="POST"&&url.pathname==="/api/v1/live-world/opt-in"){
      const flagsBlocked=await requireFlags(env,["live_world_enabled"]);if(flagsBlocked)return flagsBlocked;const user=await verifySupabaseUser(request,env);if(!user)return Response.json({error:"invalid_account_session"},{status:401});try{const optedInAt=await new LiveWorldService({url:env.SUPABASE_URL,serviceKey:env.SUPABASE_SERVICE_ROLE_KEY,fetcher:env.FETCHER||fetch}).optIn(user.id);return Response.json({optedInAt});}catch(error){return Response.json({error:error.message},{status:error.message==="live_world_requires_premium"?403:400});}
    }
    ...
    if(request.method==="GET"&&url.pathname==="/api/v1/live-world/nearby"){
const flagsBlocked=await requireFlags(env,["live_world_enabled"]);if(flagsBlocked)return flagsBlocked;const user=await verifySupabaseUser(request,env);if(!user)return Response.json({error:"invalid_account_session"},{status:401});try{const service=new LiveWorldService({url:env.SUPABASE_URL,serviceKey:env.SUPABASE_SERVICE_ROLE_KEY,fetcher:env.FETCHER||fetch}),[nearby,placement]=await Promise.all([service.nearby(user.id),service.myPlacement(user.id)]);return Response.json({nearby,placement});}catch(error){return Response.json({error:error.message},{status:400});}
    }
    if(request.method==="POST"&&url.pathname==="/api/v1/live-world/chat-requests"){
      const flagsBlocked=await requireFlags(env,["live_world_enabled"]);if(flagsBlocked)return flagsBlocked;const user=await verifySupabaseUser(request,env);if(!user)return Response.json({error:"invalid_account_session"},{status:401});const limited=await enforceRateLimit(request,env,"live_world_chat_request",user.id);if(limited)return limited;const body=await request.json().catch(()=>({}));if(typeof body.toPublicId!=="string"||!/^[0-9a-fA-F-]{36}$/.test(body.toPublicId))return Response.json({error:"invalid_live_world_target"},{status:400});try{const request_=await new LiveWorldService({url:env.SUPABASE_URL,serviceKey:env.SUPABASE_SERVICE_ROLE_KEY,fetcher:env.FETCHER||fetch}).requestChat(user.id,body.toPublicId);return Response.json({request:request_});}catch(error){return Response.json({error:error.message},{status:400});}
    }
    if(request.method==="GET"&&url.pathname==="/api/v1/live-world/chat-requests"){
      const flagsBlocked=await requireFlags(env,["live_world_enabled"]);if(flagsBlocked)return flagsBlocked;const user=await verifySupabaseUser(request,env);if(!user)return Response.json({error:"invalid_account_session"},{status:401});try{const requests=await new LiveWorldService({url:env.SUPABASE_URL,serviceKey:env.SUPABASE_SERVICE_ROLE_KEY,fetcher:env.FETCHER||fetch}).myRequests(user.id);return Response.json(requests);}catch(error){return Response.json({error:error.message},{status:400});}
    }
    const liveWorldRespondMatch=url.pathname.match(/^\/api\/v1\/live-world\/chat-requests\/([0-9a-fA-F-]{36})\/respond$/);
    if(request.method==="POST"&&liveWorldRespondMatch){
      const flagsBlocked=await requireFlags(env,["live_world_enabled"]);if(flagsBlocked)return flagsBlocked;const user=await verifySupabaseUser(request,env);if(!user)return Response.json({error:"invalid_account_session"},{status:401});const body=await request.json().catch(()=>({}));try{const result=await new LiveWorldService({url:env.SUPABASE_URL,serviceKey:env.SUPABASE_SERVICE_ROLE_KEY,fetcher:env.FETCHER||fetch}).respondChat(user.id,liveWorldRespondMatch[1],body.accept===true);return Response.json({request:result});}catch(error){return Response.json({error:error.message},{status:400});}
    }
    if(request.method==="POST"&&url.pathname==="/api/v1/live-world/claim-session"){
      const flagsBlocked=await requireFlags(env,["live_world_enabled"]);if(flagsBlocked)return flagsBlocked;const claims=await anonymousClaims(request,env);if(!claims)return Response.json({error:"invalid_anonymous_session"},{status:401});const accountToken=request.headers.get("x-account-authorization"),accountRequest=new Request(request.url,{headers:{authorization:accountToken||""}}),user=accountToken?await verifySupabaseUser(accountRequest,env):null;if(!user)return Response.json({error:"invalid_account_session"},{status:401});const body=await request.json().catch(()=>({}));if(typeof body.sessionId!=="string"||!/^[0-9a-fA-F-]{36}$/.test(body.sessionId))return Response.json({error:"invalid_session_id"},{status:400});const profileResponse=await supabaseRest(env,user,`/profiles?select=public_id&user_id=eq.${encodeURIComponent(user.id)}&limit=1`),[profile]=profileResponse.ok?await profileResponse.json():[];const claimResponse=await shard(env,"MATCHMAKING").fetch("https://match.internal/liveworld/claim",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({identityId:claims.sub,accountUserId:user.id,accountPublicId:profile?.public_id||null,sessionId:body.sessionId})});const claimData=await claimResponse.json();return Response.json(claimData,{status:claimResponse.status});
    }
    ...
    if(url.pathname==="/api/v1/arena/socket"){
      if(request.headers.get("Upgrade")?.toLowerCase()!=="websocket")return Response.json({error:"websocket_upgrade_required"},{status:426});
      const flagsBlocked=await requireFlags(env,["arena_enabled"]);if(flagsBlocked)return flagsBlocked;
      const claims=await anonymousClaims(request,env,webSocketToken(request));if(!claims)return Response.json({error:"invalid_anonymous_session"},{status:401});
      const accountToken=url.searchParams.get("accountToken"),accountRequest=accountToken?new Request(request.url,{headers:{authorization:`Bearer ${accountToken}`}}):null,account=accountRequest?await verifySupabaseUser(accountRequest,env):null;
      if(!account)return Response.json({error:"invalid_account_session"},{status:401});
      const id=env.ARENA_LOBBY.idFromName("global");
      const internal=new URL("https://arena-lobby.internal/socket"),headers=new Headers(request.headers);internal.searchParams.set("participantId",claims.sub);internal.searchParams.set("accountUserId",account.id);headers.delete("authorization");
      return env.ARENA_LOBBY.get(id).fetch(new Request(internal,{method:"GET",headers}));
    }
    ...
    if (match) {
      ...
      const internal=new URL(`https://chat.internal${url.pathname}`),headers=new Headers(request.headers);internal.searchParams.set("identityId",claims.sub);if(authorization.accountUserId)internal.searchParams.set("accountUserId",authorization.accountUserId);if(authorization.accountPublicId)internal.searchParams.set("accountPublicId",authorization.accountPublicId);if(authorization.virtual)internal.searchParams.set("virtual","1");if(authorization.virtualPeer)internal.searchParams.set("virtualPeer",JSON.stringify(authorization.virtualPeer));if(authorization.mode)internal.searchParams.set("mode",authorization.mode);if(authorization.experienceType)internal.searchParams.set("experienceType",authorization.experienceType);if(url.searchParams.get("resumeToken"))internal.searchParams.set("resumeToken",url.searchParams.get("resumeToken"));headers.delete("authorization");headers.set("sec-websocket-protocol","random-chat.v1");return env.CHAT_SESSION.get(id).fetch(new Request(internal,{method:"GET",headers}));
    }
...
export { AnonymousIdentityShard,ChatSession,DeviceOwnershipShard,MatchmakingShard,PartyRoomShard,PresenceShard,RateLimitShard,ListenerRegistryShard,ForumRoomShard,ArenaLobbyShard };
