...
async function proxyJson(request,env,path,{authorizeSearch=false,activity}={}){
  const claims=await anonymousClaims(request,env);if(!claims)return Response.json({error:"invalid_anonymous_session"},{status:401});const anon=await anonymousStub(env),body=await request.json();
  if(authorizeSearch){try{const preferences=normalizePaidPreferences(body.preferences||{});body.preferences=preferences;const accountToken=request.headers.get("x-account-authorization"),accountRequest=new Request(request.url,{headers:{authorization:accountToken||""}}),user=accountToken?await verifySupabaseUser(accountRequest,env):null;if(hasPaidPreferences(preferences)&&!user?.emailVerified)return Response.json({error:"verified_account_required_for_preferences"},{status:403});if(user){body.accountUserId=user.id;const [blocks,profiles]=await Promise.all([supabaseRest(env,user,"/blocks?select=blocked_ref"),supabaseRest(env,user,`/profiles?select=public_id&user_id=eq.${encodeURIComponent(user.id)}&limit=1`)]);if(blocks.ok)body.blockedRefs=(await blocks.json()).map(item=>item.blocked_ref);if(profiles.ok)body.accountPublicId=(await profiles.json())[0]?.public_id||null;}const restrictions=await new RestrictionService(env,env.FETCHER||fetch).find([claims.sub,body.accountUserId,body.accountPublicId]);if(restrictions.length)return Response.json({error:"account_restricted",status:restrictions[0].status},{status:403});const allowed=await anon.fetch("https://anonymous.internal/authorize-search",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({identityId:claims.sub,registered:Boolean(user?.emailVerified)})});if(!allowed.ok)return allowed;}catch(error){return Response.json({error:error.message},{status:400});}}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/v1/health") return Response.json({ ok: true, service: "random-chat-worker", phase: 21 });
    ...
    if(url.pathname.startsWith("/api/v1/admin/")){
      const authorization=await adminUser(request,env);if(authorization.error)return authorization.error;const user=authorization.user,config=new ConfigService(env,env.FETCHER||fetch),admin=new AdminService(env,env.FETCHER||fetch);try{
        ...
        if(request.method==="GET"&&url.pathname==="/api/v1/admin/dashboard"){const [persistent,presenceResponse,matchResponse]=await Promise.all([admin.dashboard(),shard(env,"PRESENCE").fetch("https://presence.internal/stats"),shard(env,"MATCHMAKING").fetch("https://match.internal/admin/stats")]);return Response.json({persistent:Array.isArray(persistent)?persistent[0]:persistent,presence:await presenceResponse.json(),matchmaking:await matchResponse.json()});}
    ...
    if(request.method==="POST"&&url.pathname==="/api/v1/presence/heartbeat"){const claims=await anonymousClaims(request,env);if(!claims)return Response.json({error:"invalid_anonymous_session"},{status:401});const body=await request.json(),accountToken=request.headers.get("x-account-authorization"),accountRequest=new Request(request.url,{headers:{authorization:accountToken||""}}),account=accountToken?await verifySupabaseUser(accountRequest,env):null;return shard(env,"PRESENCE").fetch("https://presence.internal/status",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({...body,identityId:claims.sub,anonymous:!account?.emailVerified})});}
