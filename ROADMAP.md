...
- **Not yet done**: push `202608260018_trust_safety.sql` and `202608270019_avatar_upload.sql` to the live Supabase project; create the Razorpay streaming-membership plan and set `RAZORPAY_STREAMING_PLAN_ID` in `worker/wrangler.toml`/secrets; browser-verify the whole flow (subscription checkout, banner, blur, verification, avatar upload) — per standing instruction, not run through the automated test suite, only `node --check`.
...
- **Not yet done**: push `202608270021_games_wheel.sql` to the live Supabase project (same outstanding step as the two Trust & Safety migrations); admin UI to tune `wheel_segments`; building out real payout processing when the user is ready to offer it.
...
- All new/edited files verified via `node --check` (syntax only — not yet exercised against a running Supabase instance or `wrangler dev`; no UI click-through done this slice).
- **Not yet done**: push `202608270022_games_jackpot.sql` to the live Supabase project; admin visibility into `platform_revenue_ledger`/round history; building out real payout processing when the user is ready to offer it.
...
- All new/edited files verified via `node --check` (syntax only — not yet exercised against a running Supabase instance or `wrangler dev`; no UI click-through done this slice).
- **Not yet done**: push `202608270023_games_trivia.sql` to the live Supabase project; rotating/larger question bank (fixed 5-question set is a placeholder); admin UI for authoring daily questions; Coin Tower and prediction pools (explicitly fast-follow, not MVP-blocking) — this closes out the originally agreed MVP build order (shared engine → Wheel → Jackpot → Trivia).
...
- **Not yet done**: admin UI to edit `wheel_segments` odds or author Daily Trivia questions (still DB-only); this view is read-only by design, matching the "never quietly rig" principle — round math stays inspectable, not editable, from here.
...
- **Not yet done**: pushing this migration (and the three prior games migrations) to live Supabase.
...
## Digital store — "Coming soon" placeholder, 2026-08-27

User wants a future digital store for buying/selling premium artwork (and possibly other digital goods). Not scoped in detail yet — for now, add a "Coming soon" entry point only (e.g. a disabled/teaser nav item or section) once picked up; no backend/marketplace logic yet.

## Up next
- Games platform — see scoping section above; blocked on the user answering the open questions in `QUESTIONS.md` (currency name in particular) before any implementation starts.
- Digital store (artwork buy/sell) — "Coming soon" placeholder only, not yet scoped.
- Trust & safety follow-ups above (Razorpay plan creation, migration push, browser verification of subscription/verification/avatar flows).
