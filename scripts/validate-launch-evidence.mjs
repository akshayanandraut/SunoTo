...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){const file=process.argv[2],expectedRevision=process.argv[3]||process.env.RELEASE_REVISION;if(!file||!/^[0-9a-f]{40}$/i.test(expectedRevision||"")){console.error("Usage: node scripts/validate-launch-evidence.mjs <evidence.json> <expected-release-sha>");process.exitCode=1;}else{readFile(file,"utf8").then(JSON.parse).then(value=>validateLaunchEvidence(value,{expectedRevision})).then(result=>{if(!result.ok){console.error("Phase 24 evidence is incomplete:");for(const error of result.errors)console.error(`- ${error}`);process.exitCode=1;}else console.log("Phase 24 launch evidence matches the expected release commit.");}).catch(error=>{console.error(error.message);process.exitCode=1;});}}
