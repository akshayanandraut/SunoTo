alter table public.party_rooms drop constraint if exists party_rooms_room_type_check;
alter table public.party_rooms add constraint party_rooms_room_type_check check (room_type in ('audio','audio_video','music','radio'));
alter table public.party_rooms add column if not exists is_public boolean not null default false;
create index if not exists party_rooms_public_idx on public.party_rooms(room_type,status) where is_public;

create policy "public radio rooms are discoverable" on public.party_rooms for select to authenticated using (
  is_public and status='active' and room_type='radio'
);
...
returns table(public_id uuid,name text,join_code text,room_type text)
language sql security definer set search_path='' stable as $$
  select public_id,name,join_code,room_type from public.party_rooms
  where is_public and status='active' and room_type='radio'
