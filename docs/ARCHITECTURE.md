...
Phase 22 writes only an opaque idempotency key, allowlisted event name, coarse dimension and integer value before incrementing an IST-day aggregate. It stores no event payload, message text or analytics user profile. Server result IDs deduplicate retries across match, wallet and room flows. The admin reads exact 30-day aggregates; the public snapshot separates human and virtual matches and rounds down so displayed activity never exceeds the underlying count.

Phase 23 places a keyed Durable Object rate-limit boundary around anonymous sessions, searches, payments, analytics and grievances, plus a temporary per-room message limiter that is deleted when the room ends. The Worker wrapper rejects unknown browser origins, handles preflight centrally, adds browser defenses, disables private caching and converts unexpected failures to reference-only errors. Static deployment headers enforce CSP and HSTS. Account exports are assembled from business tables and cannot include chat text; deletion/grievance requests have RLS-disabled public tables, service-role-only writes and locked/audited admin transitions. External SMTP, legal approval, backup and release evidence remain fail-closed production gates rather than repository secrets.
